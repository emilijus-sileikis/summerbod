<?php include('partials/menu.php'); ?>
<!-- Health section start -->
<link rel="stylesheet" href="/codeigniter4/assets/css/sidenav.css">
<?php include('sidenavmenu.php'); ?>

<div class="container">
    <h2 style="color: black; margin-left: 20%; margin-top:10%" class="text-center">Triceps Exercises</h2>  <!-- Input muscle group -->
    <div class="workout-menu-box">
	<div class="workout-menu-img">
        <?php include('partials/base64/random-menu.php'); ?>                                  <!-- Input photo by id -->
    </div>
        <div class="workout-menu-desc">
            <h3>Triceps</h3>                                                                 <!-- Input title by id -->
            <p class="workout-detail">
            Muscle group: Triceps </br> </br>                                              <!-- Input details by id -->
		    Category: Home workout </br> </br>
		    Difficulty: Begginer 
            </p>
        </div>
	    <div class="clearfix"></div>
    </div>
</div>

<div class="container">
    <div class="workout-menu-box">
	<div class="workout-menu-img">
        <?php include('partials/base64/random-menu.php'); ?>                                  <!-- Input photo by id -->
    </div>
        <div class="workout-menu-desc">
            <h3>Yuh</h3>                                                                 <!-- Input title by id -->
            <p class="workout-detail">
            Muscle group: Triceps </br> </br>                                              <!-- Input details by id -->
		    Category: Home workout </br> </br>
		    Difficulty: Begginer 
            </p>
        </div>
	    <div class="clearfix"></div>
    </div>
</div>

<div class="container">
    <div class="workout-menu-box">
	<div class="workout-menu-img">
        <?php include('partials/base64/random-menu.php'); ?>                                  <!-- Input photo by id -->
    </div>
        <div class="workout-menu-desc">
            <h3>Bruh</h3>                                                                 <!-- Input title by id -->
            <p class="workout-detail">
            Muscle group: Triceps </br> </br>                                              <!-- Input details by id -->
		    Category: Home workout </br> </br>
		    Difficulty: Begginer 
            </p>
        </div>
	    <div class="clearfix"></div>
    </div>
</div>

<!-- Workouts section end -->
    <?php include('partials/footer.php'); ?>